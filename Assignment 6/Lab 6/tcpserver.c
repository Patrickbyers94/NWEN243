...
int main(int argc, char *argv[]){
  ...
  while(1){
    msgsock = accept(sock,(struct sockaddr*) &server,(socklen_t *) &length); //accept an incoming connection
    if(msgsock<0){ //check if the connection was accepted correctly
  ...
  //  count++;
    ...
    id = fork();
   //printf("id is %d \n",id);
    if(id == 0){ //check if this is a child process
    if(read(msgsock,buf,bufsize)<0){ //read the message from the client and ensure it is successful
    	printf(stderr, "Failed to read the data \n");
    	close(msgsock);
    	//count--;
    	return(-1);
    }
	for (char *p = buf ; *p ; *p = toupper(*p), p++); //convert the characters in the buffer to uppercase
	if(write(msgsock,buf,bufsize)<0){ //write the message to the client and check it is successful 
	...
	return(-1);
	 ...
	 else if(id>0){ // check if this is a parent process and if so continue
	 continue;
	 }
	 else { //if not parnet or child an error has occured
