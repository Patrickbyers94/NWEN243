...
int main(argc, argv) int argc; char *argv[];{
 ...
 while(1==1){
  ...
  fprintf(stderr, "failed to connect %s to %s which is of length %s",sock);
  close(sock);
  return(-1);}
  ...
  fprintf(stderr, "failed to send message %s to %s using buffer size %s",buf,sock,bufsize);
  close(sock);
  return(-1);}

  // read response from server
  if(read(sock,buf,bufsize)<0){
  fprintf(stderr, "failed to read message from %s to the buffer using size %s",sock,bufsize);
  close(sock);
  return(-1);}
